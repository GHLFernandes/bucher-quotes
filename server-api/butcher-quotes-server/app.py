...
BillyButcherApi = Flask(__name__)
cors = CORS(BillyButcherApi)
BillyButcherApi.config['CORS_HEADERS'] = 'Content-Type'
...
@cross_origin()
@BillyButcherApi.route('/', methods=['GET'])
def dashboard():
    return jsonify(get_random_quote())

if __name__ == '__main__':
    BillyButcherApi.run(host= '127.0.0.1',port=5000)
